...
def multi_label_spurious_transform(labels_to_modify: list, dataset, modifier, text_proportion: float, seed=None):
    """
    Applies a transformation to a subset of texts in the dataset that have any of the specified labels.
    This is useful for modifiers that can handle multiple labels (like GoodBadInjection).

    Args:
        labels_to_modify (list): List of labels to modify.
        dataset: The dataset containing the text data.
        modifier: An instance of a Modifier subclass that modifies (text, label).
        text_proportion (float): Proportion of texts to transform using the modifier (between 0 and 1).
        seed (int, optional): Seed for random sampling reproducibility.
    ...
    Returns:
        Dataset: A new dataset with the transformations applied to examples with the given labels.
    """
    # Filter dataset to get examples with any of the specified labels
    dataset_to_modify = dataset.filter(lambda example: example["labels"] in labels_to_modify)
    remaining_dataset = dataset.filter(lambda example: example["labels"] not in labels_to_modify)
